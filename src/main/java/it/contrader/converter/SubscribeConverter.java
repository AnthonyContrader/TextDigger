...
public class SubscribeConverter extends AbstractConverter<Subscribe, SubscribeDTO> {
	...
	public Subscribe toEntity(SubscribeDTO subscribeDto) {
		...
		if(subscribeDto != null) {
			subscribe = new Subscribe(subscribeDto.getId(), subscribeDto.getText(), subscribeDto.getUser(), subscribeDto.getInterestgroup());
		}
		return subscribe;
	}
	...
	public SubscribeDTO toDTO(Subscribe subscribe) {
		...
		if(subscribe != null) {
			subscribeDTO = new SubscribeDTO(subscribe.getId(), subscribe.getText(), subscribe.getUser(), subscribe.getInterestgroup());
		}
		return subscribeDTO;
	}
